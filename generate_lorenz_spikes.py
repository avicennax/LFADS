...
def main(conditions_num=30, neuron_num=30, trials_num=20, seed_std_dev=30, 
    ...
    for seed in seeds:
        ...
        for _ in range(trials_num):
            x = np.array([np.random.poisson(rate) for rate in rates.flatten()]).reshape(
                steps - burn_in, neuron_num)
            spikes.append((x.T > 0).astype(float))
    ...
    if build_training_matrix:
        all_spikes = []
        for spikes_dict in data_dict['seeds']:
            all_spikes += spikes_dict['spikes']
        X = np.array(all_spikes)
        X = X.reshape([X.shape[0], X.shape[2], X.shape[1]])
        np.save('data/lorenz_data_matrix-' + timestamp, X)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--burn-in', type=int, default=100, 
        help='he number of steps to ignore for latent factors; ' 
             'employed to wash-out initial state.')
    parser.add_argument('--build-training-matrix', action='store_true',
        help='Save training matrix derived from data dictionary.')
